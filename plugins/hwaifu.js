...
const isNsfwCommand = (cmd) => ['hwaifu', 'sex', 'xxx'].includes(cmd);
...
const getNsfwImage = async (m, sock) => {
  ...
  const botNumber = config.SUDO_NUMBER;  // The bot's phone number (without the + sign, e.g. "94741259325")
  const ownerNumber = config.OWNER_NUMBER;  // The bot owner's phone number (without the + sign, e.g. "94741259325")
  ...
  if (cmd === 'hwaifu') {
    ...
    endpoint = 'nsfw/sex';
    caption = 'Here is your random NSFW Sex image!';
  } else if (cmd === 'xxx') {
    endpoint = 'nsfw/xxx';
    caption = 'Here is your random NSFW Xxx image!';
  }
