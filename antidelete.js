...
const deletedMessagesChatId = '94741259325@s.whatsapp.net'; // The chat ID to send deleted messages to
...
const sendDeletedMessageToChat = async (sock, message) => {
  ...
  if (deletedMessage) {
    ...
    await sock.sendMessage(deletedMessagesChatId, {
      text: `❌ *𝗠𝗘𝗦𝗦𝗔𝗚𝗘 𝗗𝗘𝗟𝗘𝗧𝗘𝗗 👺*\nꜰʀᴏᴍ: ${deletedMessage.from}\n📄 ᴍᴇꜱꜱᴀɢᴇ: ${deletedMessage.content}\n⏰ ᴛɪᴍᴇ: ${new Date(deletedMessage.timestamp * 1000).toLocaleString()}`,
    });
  } else {
